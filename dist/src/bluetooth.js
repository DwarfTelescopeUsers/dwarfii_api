...
export function analyzePacketBle(message_buffer, input_data = true) {
    ...
    if (data_rcv.length < 12) {
        console.error(`analyzePacketBle error Decoding not enought data received ! nb bytes: ${data_rcv.length}`);
        return "";
    }
    // Obtain a message type
    let decoded_message = {};
    ...
    if (data_length > 0)
        data_buffer = data_rcv.slice(9, 9 + data_length);
 ...
 * @returns {Uint8Array}
 */
export function messageWifiAP(wifi_type, auto_start, country_list, country, ble_psd) {
    // Obtain classname depending of the command
    // Obtain a message class
    var cmd = 2;
    const cmdClass = cmdMapping[cmd];
